...
const assertNum = (v, name) => {
    if (typeof v !== 'number' || Number.isNaN(v))
        bad(`${name} phải là số`, 400);
};
const assertNonNegative = (v, name) => {
    if (v < 0)
        bad(`${name} không được âm`, 400);
};
 ...
 */
const ensureAccessByRoles = async (teamId, userId, allowedRoles, includeOwnerId = true) => {
    ...
    const hasRole = allowedRoles.length
        ...
        : false;
    const can = hasRole || (includeOwnerId && team.ownerId === userId);
    if (!can)
        bad(`Bạn không có quyền thực hiện thao tác này (cần role: ${allowedRoles.join(', ')})`, 403);
};
// ====== Public Service APIs ======
...
/** setCategories: admin */
const setFinanceCategories = async (teamId, userId, newCategory) => {
...
const renameWorkspaceName = async (teamId, userId, newName) => {
    assertTeamName(newName);
    await ensureAccessByRoles(teamId, userId, ['owner', 'admin']);
    return TeamModel.updateName(teamId, newName.trim());
};
const permitReportAccess = async (teamId, userId, allow) => {
...
const checkValidInvite = async (teamId, email, inviterID) => {
...
const sendInviteEmail = async (teamId, email, inviterID) => {
    if (!teamId || !email || !inviterID)
        bad('Thiếu teamId, email hoặc inviterID', 400);
    const team = await TeamModel.findById(teamId);
    if (!team)
        throw new Error('Team không tồn tại');
    const inviter = await UserModel.findById(inviterID);
    if (!inviter)
        throw new Error('Người mời không tồn tại');
...
const handleInviteResponse = async (inviteToken, email) => {
...
const calculateBalance = async (teamId) => {
...
const removeMember = async (teamId, memberId, userId) => {
    if (!teamId || !memberId)
        bad('Thiếu teamId hoặc memberId', 400);
    ...
    if (!kickerMembership || !kickedMembership)
        bad('Thành viên không tồn tại trong nhóm', 404);
...
const changeMemberRole = async (teamId, memberId, userId, newRole) => {
    if (!teamId || !memberId || !newRole)
        bad('Thiếu teamId, memberId hoặc newRole', 400);
